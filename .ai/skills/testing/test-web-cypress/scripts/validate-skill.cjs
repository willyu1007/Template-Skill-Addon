 ...
 *
 * Usage:
 *   node scripts/validate-skill.cjs
 */
const fs = require('fs');
...
function parseFrontMatter(md) {
  // Minimal YAML front matter parser for flat "key: value" pairs.
  // Supports quoted values and ':' within values.
  ...
  for (const line of raw.split('\n')) {
    ...
    if (!m) {
      return { error: `Invalid front matter line: "${line}"` };
    }
    const k = m[1];
    let v = m[2] ?? '';
    v = v.trim();
...
function requireSection(md, heading) {
  // Very small heuristic: ensure "## <heading>" exists.
...
function extractBacktickPaths(md) {
  ...
  while ((m = re.exec(md)) !== null) {
    ...
    if (!v) continue;
    if (v.startsWith('reference/') || v.startsWith('scripts/') || v.startsWith('assets/')) {
      paths.add(v);
    }
  }

  // Markdown links: [text](reference/...)
  const linkRe = /\[[^\]]*\]\(([^)]+)\)/g;
...
function main() {
  ...
  // Prefer Claude-compatible names to reduce friction across agents.
  if (!/^[a-z0-9-]+$/.test(front.name)) {
    die(`Invalid skill name "${front.name}". Use only [a-z0-9-] for portability.`);
  }

  if (front.name !== skillNameDir) {
    die(`Skill name mismatch. front matter name="${front.name}" but directory="${skillNameDir}".`);
  }

  // Validate referenced local files exist.
  ...
  if (missing.length) {
    die(`SKILL.md references missing files:\n- ${missing.join('\n- ')}`);
  }
  ...
  for (const p of procs) {
    ...
    // Check recommended sections (warn but don't fail)
  ...
  if (procWarnings > 0) {
    console.log(colors.yellow(`- Warnings: ${procWarnings} recommended section(s) missing`));
  }
}
