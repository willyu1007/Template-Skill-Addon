...
```bash
python scripts/skillgen.py init-plan   --inputs "docs/**/*.md"   --out conversion-plan.json   --skills-root out/skills   --layout categorized   --tier1 workflows   --tier2 common
```
...
```bash
python scripts/skillgen.py apply --plan conversion-plan.json
```
...
## 5) Package (optional)
```bash
python scripts/skillgen.py package   --skills-root out/skills   --out skills-bundle.zip   --overwrite
```
