...
async function main() {
  ...
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log(
      [
        "collect_evidence.cjs",
        "",
        "Reads JSON from stdin and writes evidence JSON to stdout.",
        "",
        "Examples:",
        "  # from repo root",
        "  node .ai/skills/workflows/llm/debug-mode/scripts/collect_evidence.cjs < input.json",
        "  # if cwd is .ai/skills/workflows/llm/debug-mode",
        "  node scripts/collect_evidence.cjs < input.json",
        "",
        "Run tests:",
        "  node --test .ai/skills/workflows/llm/debug-mode/scripts/collect_evidence.test.cjs",
      ].join("\n"),
    );
    return;
  }
