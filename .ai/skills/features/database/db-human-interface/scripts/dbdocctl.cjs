...
function tryRunDbssotctl(repoRoot) {
  ...
  if (!exists(dbssotctl)) return { ran: false, ok: false, note: 'dbssotctl.js not found' };
  ...
  if (exists(prismaPath)) {
    return {
      ...
      error: `No normalized contract found at ${ssotCfg.paths.dbContextContract}.\n` +
        ...
        `Run: node .ai/scripts/dbssotctl.js sync-to-context (requires template scripts).`
    };
  }
...
function renderColumnCrossTableDoc({ schemaMeta, columnName, matches }) {
  ...
  const guidance = `## Next steps\n\n- To see a full table view, run:\n  - \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query <Table>\`\n`;
...
function renderSearchDoc({ schemaMeta, term, candidates }) {
  ...
  const guidance = `## How to proceed\n\n- If you meant a table: run \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query <TableName>\`\n- If you meant a column: run \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query <ColumnName>\`\n- If ambiguous, pick one of the results above and re-run \`query\`.\n`;
...
function renderConceptDoc({ schemaMeta, idx, cluster }) {
  ...
  const next = [
    ...
    `  - \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query "${cluster.term}" --view graph\``,
    '- To draft a change for this concept cluster:',
    `  - \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs modify "${cluster.term}" --scope concept\``
  ].join('\n');
...
function renderApiDoc({ schemaMeta, table }) {
  ...
  const guidance = [
    ...
    `  - \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query ${table.name} --view table\``
  ].join('\n');
...
function renderConceptModifyDoc({ schemaMeta, cluster, existingDbops }) {
  ...
  const editable = [
    ...
    '## Next steps',
    '',
    `- Run: \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs plan "${cluster.term}"\``
  ].join('\n');
...
function renderModifyDoc({ schemaMeta, idx, table, existingDbops }) {
  ...
  const editable = [
    ...
    '## Next steps',
    '',
    `- Run: \`node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs plan ${table.name}\``
  ].join('\n');
...
function printHelp() {
  const msg = `dbdocctl.cjs — Human-friendly DB structure and change drafting\n\n` +
    `Usage:\n` +
    `  node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs status\n` +
    `  node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs search <term>\n` +
    `  node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query <object> [--view table|concept|graph|api] [--depth <n>] [--max-tables <n>]\n` +
    `  node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs modify <object> [--scope table|concept] [--depth <n>] [--max-tables <n>]\n` +
    `  node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs plan <object>\n\n` +
    `Artifacts:\n` +
    ...
    if (view === 'concept' || view === 'graph') {
      ...
      if (!table) {
        ...
        doc = [
          ...
          '  - `node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs query <TableName> --view api`'
        ].join('\n');
      } else {
    ...
    if (scope === 'concept') {
      ...
      if (!cluster.ok) {
        ...
        const doc = [
          ...
          '  - `node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs modify <TableName>`'
        ].join('\n');
    ...
    if (!modifyPath) {
      console.error(
        `Modify doc not found for '${term}'.\n` +
          `Run one of:\n` +
          `- node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs modify "${term}"\n` +
          `- node .ai/skills/features/database/db-human-interface/scripts/dbdocctl.cjs modify "${term}" --scope concept`
      );
      return EXIT.FAILED;
    }
