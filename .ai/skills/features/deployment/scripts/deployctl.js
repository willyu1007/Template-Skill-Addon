...
function usage(exitCode = 0) {
  const msg = `
Usage:
  node .ai/skills/features/deployment/scripts/deployctl.js <command> [options]
...
Examples:
  node .ai/skills/features/deployment/scripts/deployctl.js init --model k8s --k8s-tool helm
  node .ai/skills/features/deployment/scripts/deployctl.js add-service --id api --artifact ghcr.io/acme/api:v1.2.3
...
function cmdListServices(repoRoot, format) {
  ...
  if (config.services.length === 0) {
    ...
    console.log('  Run: node .ai/skills/features/deployment/scripts/deployctl.js add-service --id api --artifact ghcr.io/acme/api:v1.2.3');
    return;
  }
...
function cmdPlan(repoRoot, serviceId, envId, tagOverride) {
  ...
  if (config.model === 'k8s') {
    ...
    console.log('# Optional: verify health (adjust URL)\nnode ops/deploy/scripts/healthcheck.js --url https://example.com/health\n');
  } else {
