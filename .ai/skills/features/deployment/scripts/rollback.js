 ...
 *
 * Usage:
 *   node .ai/skills/features/deployment/scripts/rollback.js --service <service> --env <env>
 */
...
`);
  ...
  if (!config) {
    ...
    console.error('Run: node .ai/skills/features/deployment/scripts/deployctl.js init');
    return 1;
  }
  ...
  if (!svc) {
    ...
    if (services.length > 0) {
      ...
      console.error('No services are registered yet. Run: node .ai/skills/features/deployment/scripts/deployctl.js add-service --id <id> --artifact <ref>');
    }
    return 1;
  }
