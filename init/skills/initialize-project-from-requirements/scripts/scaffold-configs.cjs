 ...
 * 
 * Usage:
 *   node scaffold-configs.cjs --blueprint <path> --repo-root <path> [--apply]
 */
...
// Export for init-pipeline.cjs (single source of truth)
function generateConfigFiles(repoRoot, blueprint, apply, force = false) {
  return generateConfigs(blueprint, repoRoot, apply, force);
}

module.exports = { generateConfigFiles, generateConfigs };
