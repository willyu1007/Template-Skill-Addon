...
- Root `README.md` generated from the blueprint (overwrites existing) when `scripts/templates/README.template.md` is present
...
Stage C `apply` generates a project-specific root `README.md` from the blueprint via `scripts/templates/README.template.md`. Include `README.md` in the Stage C review.
...
If user replies `"update agents"`, update the root `AGENTS.md` with project-specific info. Show a diff and request explicit approval before writing. See `## Post-init: Update AGENTS.md` in this file for rules.

### 5) Optional: remove the init kit
